...
(function () {
    ...
    // 2.5、判断是否是fashion下的内容页
    function isFashion() {
        var tagA = document.querySelector('#topBar ul li:nth-child(2) a')
        if (!tagA) {
            return false
        } else {
            if (tagA.href && tagA.href.indexOf('fashion') !== -1) {
                return true
            } else {
                return false
            }
    ...
    var params = {
        ...
        site: config.site,
        type: config.type,
        strategy: strategy || 'random'//palcloset.jp ragtag.jp random
    }
    ajax({
        url: config.dataUrl,
        data: params,
