...
/* 获取用户当前使用设备（pc or mobile） */
const getDevice = function (ua) {
...
const getDate = function () {
    ...
    let day = nowDate.getDate() < 10 ? '0' + nowDate.getDate() : String(nowDate
        .getDate());
    let t = year + month + day;
    return t
}
/* 更改site名字 */
const modifySiteName = function (site) {
    if (site === 'mix.tokyo'){
        return site
    }
    return site.replace(/(\w+).(\w+)/,function(match,m1){
        return m1
    })
}
/* 扩展方法 */
